...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val x = w.toLowerCase().toSeq.groupBy(c => c)

    val z = for ((c, occur) <- x) yield (c, occur.length)
    z.toList.sorted
  }
  ...
  def wordAnagrams(word: Word): List[Word] = {
    ...
    dictionaryByOccurrences.getOrElse(occurrences, List(word))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      def tailCombos = combinations(occurrences.tail)
      (for {
        combo <- tailCombos
        count <- 1 to head._2
      } yield (head._1, count) :: combo) ++ tailCombos
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.toMap.foldLeft(x.toMap)(
      (acc, p) => acc.updated(p._1, acc(p._1) - p._2)
